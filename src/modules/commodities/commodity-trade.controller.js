...
const MARGIN_MULTIPLIER = 10; // Typical commodity leverage
const DEFAULT_CHARGES = 2.0; // Flat charge per trade

export async function buyCommodity(req, res) {
  const { symbol, quantity, entryPrice, timeFrame = 'INTRADAY' } = req.validatedBody;
  ...
  try {
    ...
    const activeShortsResult = await sql(
      `SELECT * FROM commodity_positions WHERE user_id = $1 AND symbol = $2 AND status = 'ACTIVE' AND trade_type = 'SELL' ORDER BY entry_time ASC`,
    ...
    for (const position of activeShorts) {
      ...
      if (closeQty === pQuantity) {
        await sql(
          `UPDATE commodity_positions SET status = 'EXITED', exit_price = $1, exit_time = NOW(), pnl = $2, pnl_percent = $3 WHERE id = $4`,
        ...
        await sql(
          `UPDATE commodity_positions SET quantity = $1, margin_used = $2 WHERE id = $3`,
    ...
    if (closedPositionIds.length > 0) {
      ...
      await updateCommodityPerformanceMetrics(userId);
    }
    ...
    console.error(`[CommodityTrade] ERROR:`, error);
    return res.status(500).json({ message: "Failed to process buy order", error: error.message });
  }
}

export async function sellCommodity(req, res) {
  const { symbol, quantity, entryPrice, timeFrame = 'INTRADAY' } = req.validatedBody;
  const userId = req.user?.id;
  ...
  try {
    ...
    const activeBuysResult = await sql(
      `SELECT * FROM commodity_positions WHERE user_id = $1 AND symbol = $2 AND status = 'ACTIVE' AND trade_type = 'BUY' ORDER BY entry_time ASC`,
      [userId, symbol.toUpperCase()]
    );
    ...
    if (!isIntraday) {
      ...
      if (totalOwned < quantity) {
        return res.status(400).json({ message: `Delivery sell failed. You only own ${totalOwned} contracts of ${symbol}.` });
      }
    }
...
export async function getCommodityPerformanceMetrics(req, res) {
  const userId = req.user?.id;
  try {
    const result = await sql(`SELECT * FROM commodity_performance WHERE user_id = $1`, [userId]);
    if (result.rowCount === 0) {
      await updateCommodityPerformanceMetrics(userId);
      const newResult = await sql(`SELECT * FROM commodity_performance WHERE user_id = $1`, [userId]);
      return res.json(newResult.rows[0] || {});
    }
    return res.json(result.rows[0] || {});
  } catch (error) {
    console.error(`[CommodityTrade] Fetch Performance Error:`, error);
    return res.status(500).json({ message: "Failed to fetch performance" });
  }
}

export async function updateCommodityPerformanceMetrics(userId) {
  try {
    const positionsResult = await sql(
      `SELECT * FROM commodity_positions WHERE user_id = $1 AND status = 'EXITED' ORDER BY exit_time DESC`,
      [userId]
    );
    ...
    await sql(
      `INSERT INTO commodity_performance 
    ...
    console.error(`[CommodityTrade] Error updating performance:`, error);
  }
}
