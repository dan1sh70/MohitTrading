...
```javascript
const response = await fetch("http://localhost:4000/api/crypto/prices");
...
const response = await fetch("http://localhost:4000/api/crypto/buy", {
  method: "POST",
  headers: {
...
const response = await fetch("http://localhost:4000/api/crypto/portfolio", {
  headers: {
    Authorization: `Bearer ${token}`,
  },
});
...
```bash
curl http://localhost:4000/api/crypto/stats/ETHUSDT
```
